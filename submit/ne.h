...
#define PACKETSIZE \
  1024 /* assume packet size large enough to contain a full routing table */
...
#define FAILURE_DETECTION                                                  \
  (UPDATE_INTERVAL * 3) /* not receiving a nbr's update more than 3 update \
                           cycles = 3 * UPDATE_INTERVAL, consider it dead */
#define CONVERGE_TIMEOUT                                                  \
  (UPDATE_INTERVAL * 5) /* if routing table is not changed after 5 update \
                           cycles, assume converged */
...
struct pkt_INIT_REQUEST {
  unsigned int
      router_id; /* my id, used to retrieve neighbor database from ne */
};
...
struct pkt_INIT_RESPONSE {
  ...
  struct nbr_cost
      nbrcost[MAX_ROUTERS]; /* array holding the cost to each neighbor */
};
...
#ifdef PATHVECTOR
  unsigned int
      path_len; /* length of loop-free path to dest_id, eg: with path R1 -> R2
                   -> R3, the length is 3; self loop R0 -> R0 is length 1 */
  unsigned int
      path[MAX_PATH_LEN]; /* array containing id's of routers along the path,
                             this includes the source node, all intermediate
                             nodes, and the destination node; self loop R0 -> R0
                             should only contain one instance of R0 in path */
#endif
};
...
struct pkt_RT_UPDATE {
  ...
  unsigned int
      dest_id; /* id of neighbor router to which routing table is sent */
  unsigned int no_routes; /* number of routes in my routing table */
  struct route_entry
      route[MAX_ROUTERS]; /* array containing rows of routing table */
};
